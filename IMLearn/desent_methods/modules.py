...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        m = self.regularization_module_.compute_output()
        if self.include_intercept_:
            m = np.insert(m, 0, 0)
        return self.fidelity_module_.compute_output() + self.lam_ * m
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        if self.include_intercept_:
            ...
            self.regularization_module_ = self.weights[1:]
        else:
            self.fidelity_module_.weights = self.weights
            self.regularization_module_ = self.weights
